...
namespace Os
{
...
#else
		m_fd = ::open(fileName,
                      flag == IORead ? O_RDONLY : (O_WRONLY | O_CREAT),
                      S_IRWXU);
        if (m_fd < 0)
        {
            int err = errno;
...
#if K3DPLATFORM_OS_WIN
	bool MemMapFile::Open(const WCHAR * fileName, IOFlag flag)
	{
		m_FileHandle = ::CreateFileW(fileName, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL | FILE_FLAG_SEQUENTIAL_SCAN, NULL);
		if (m_FileHandle == INVALID_HANDLE_VALUE) return false;
	...
	size_t MemMapFile::Write(const void *, size_t)
	{
    ...
    int Exec(const ::k3d::kchar * cmd, ::k3d::kchar *const *argv)
    {
#if K3DPLATFORM_OS_WIN
	...
	bool MakeDir(const ::k3d::kchar * name)
	{
#if K3DPLATFORM_OS_WIN
	...
	bool Exists(const ::k3d::kchar * name)
	{
#if K3DPLATFORM_OS_WIN
	...
	bool Copy(const ::k3d::kchar * src, const ::k3d::kchar * target)
	{
#if K3DPLATFORM_OS_WIN
	...
	bool Remove(const::k3d::kchar * lpszDir)
	{
#if K3DPLATFORM_OS_WIN
		...
		do
		{
			if (wcscmp(wfd.cFileName, KT(".")) == 0 || wcscmp(wfd.cFileName, KT("..")) == 0)
			{
				continue;
			}

			_snwprintf(szFile, _countof(szFile) - 1, KT("%s%s"), szDelDir, wfd.cFileName);
	...
	bool ListFiles(const ::k3d::kchar * srcPath, PFN_FileProcessRoutine pFn)
	{
#if K3DPLATFORM_OS_WIN
		if (pFn == nullptr)
			return false;
...
#define DEFAULT_THREAD_STACK_SIZE 2048
	...
	uint64 Socket::Send(SocketHandle remote, std::string const & buffer)
	{
		return ::send(remote, buffer.c_str(), (int)buffer.size(), 0);
	}

	uint64 Socket::Send(SocketHandle remote, const char * pData, uint32 sendLen)
	{
		return ::send(remote, pData, sendLen, 0);
	}

	uint64 Socket::Receive(SocketHandle reomte, void * pData, uint32 recvLen)
	{
